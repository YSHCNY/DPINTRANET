  ...
  <div class="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md items-center justify-center">
    <div class="w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div class="border-b border-slate-200 px-6 py-5 sm:px-8">
        ...
        <p class="mt-2 text-sm leading-6 text-slate-600">Enter your username or employee ID and the registered email address to receive a verification code.</p>
      </div>

      <form method="POST" action="index.php?controller=Auth&action=forgotPassword" class="space-y-4 p-6 sm:px-8 sm:py-7">
        ...
        <button type="submit" class="h-10 w-full rounded-lg bg-slate-900 text-sm font-medium text-white transition hover:bg-slate-800">Send Verification Code</button>
        <a href="index.php?controller=Auth&action=login" class="inline-flex text-sm text-slate-500 transition hover:text-slate-700">← Back to Login</a>
      </form>
    </div>
  </div>
</div>
