            ...
            <form id="manualStaffForm" action="index.php?controller=StaffDirectory&action=store" method="post" enctype="multipart/form-data" class="space-y-4">
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="space-y-3">
                        ...
                        <input id="firm" name="firm" type="text" class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-900 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-100" placeholder="Acme Consulting">
                    </div>
                    <div class="space-y-3">
        ...
        <section class="xl:col-span-3 space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <a href="<?= rtrim(BASE_URL, '/') ?>/uploads/stafftemplate/staff_directory_template.csv" class="inline-flex h-9 items-center rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-50">Download</a>
                </div>
            </div>
                ...
                <div id="validationErrors" class="<?= empty($importErrors) ? 'hidden' : '' ?> rounded-xl border border-rose-100 bg-rose-50 p-3 text-sm text-rose-700">
                    <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <?php if (!empty($importResults['invalidRowNames'])): ?>
                                <p class="text-xs text-rose-700">
                                    Invalid rows: <?= htmlspecialchars(implode(', ', $importResults['invalidRowNames'])) ?>
                                </p>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="max-h-72 overflow-y-auto overflow-x-auto rounded-lg border border-rose-200 bg-white shadow-sm">
                        <table class="min-w-full text-left text-xs text-rose-700">
                            <thead class="bg-rose-50 sticky top-0">
                                <tr>
                                    ...
                                    <th class="px-2 py-2 font-semibold uppercase tracking-[0.2em] text-rose-700">Name</th>
                            ...
                            <tbody>
                                <?php foreach ($importErrors as $error): ?>
                                    <tr class="border-t border-rose-100 bg-white">
                                        ...
                                        <td class="px-2 py-2"><?= htmlspecialchars($error['name'] ?? '') ?></td>
